  ...
  try {
    const sql = `
      SELECT
        ...
        v.limite_km_mensal,
        (
            SELECT MAX(lk.km_atual)
            FROM leituras_km lk
            JOIN alocacoes a ON lk.id_alocacao = a.id
            WHERE a.id_veiculo = v.id AND lk.data_leitura < DATE_FORMAT(CURDATE(), '%Y-%m-01')
        ) AS km_inicio_periodo,
        (
            SELECT MIN(lk.km_atual)
            FROM leituras_km lk
            JOIN alocacoes a ON lk.id_alocacao = a.id
            WHERE a.id_veiculo = v.id AND MONTH(lk.data_leitura) = MONTH(CURDATE()) AND YEAR(lk.data_leitura) = YEAR(CURDATE())
        ) AS km_primeira_leitura_mes,
        v.km_atual AS km_final_periodo
      FROM
        veiculos v
    `;
    ...
    const relatorio = veiculos.map(v => {
      let kmInicial = v.km_inicio_periodo;
      if (kmInicial === null) {
        kmInicial = v.km_primeira_leitura_mes || v.km_final_periodo;
      }
